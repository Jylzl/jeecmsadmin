 ...
 * @LastAuthor: lizlong
 * @lastTime: 2019-09-06 18:00:55
 -->
<template>
	<div>
		<el-upload class="upload-attach" :action="uploadUrl" :data="data" :multiple="multiple" :limit="limit"
			:name="name" :auto-upload="auto_upload" :before-upload="beforeAvatarUpload" :on-remove="removeList"
			:on-success="onSuccess" :on-error="onError" :show-file-list="show_file_list" :file-list="fileList"
			ref="upload">
...
<script>
	...
	export default {
		...
		data() {
			return {
				uploadUrl: this.$store.state.sys.baseUrl +
			...
			beforeAvatarUpload(file) {
				...
				if (this.isDoc) {
					if (!this.isMIME(file.name)) {
						this.$message.error(
							"只能上传txt,doc,docx,xls,xlsx,ppt,pptx,pdf"
						);
						isUpLoad = false;
					} else {
						isUpLoad = true;
