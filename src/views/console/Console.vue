								...
								<div slot="header">
									...
									<ul class="tab-spot">
										<li :class="{active:sysChartItem.active==0}"
											@click="tabSpotChange('sysChartItem',0,'后台用户')">0
										</li>
								...
								<div slot="header">
									...
									<ul class="tab-spot">
										<li :class="{active:dataChartItem.active==0}"
											@click="tabSpotChange('dataChartItem',0,'后台用户')">0
										</li>
...
export default {
	...
	data() {
		...
		return {
			...
			chart1loading: false,
			chart1Data: {
				columns: ["日期", "浏览量（PV）", "统计（IP）", "访客数（UV）"],
				rows: []
			},
			//来源分析数据
			chart2loading: false,
	...
	methods: {
		...
		create(type) {
			...
			if (type === "link") {
				this.$axios
					.post(this.$api.flowSourceList, this.params)
					.then(res => {
