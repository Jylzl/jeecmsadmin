		...
		<el-main>
			...
			<div class="right-center">
				...
				<div class="right-center-right">
					...
					<label for="">发布者：</label>
					<el-input v-model="params.queryInputUsername" placeholder="请输入发布者" @change="query" size="small"
						clearable></el-input>
						...
						<el-table :data="tableData" row-key="id" ref="multipleTable" tooltip-effect="dark" size="small"
							...
							<el-table-column label="标题" prop="title" :show-overflow-tooltip="true">
								<template slot-scope="scope">
									<el-tag size="mini" class="mr-10">
										<el-link :underline="false" class="cms-channel-name" :href="scope.row.channelUrl"
											:title="scope.row.channelName" target="_blank">{{scope.row.channelName}}</el-link>
									</el-tag>
									<el-link :underline="false" class="news-link" :href="scope.row.url" :title="scope.row.title"
										target="_blank">{{scope.row.title}}</el-link>
								</template>
							</el-table-column>
							<el-table-column label="置顶" prop="topLevel" width="180" align="center">
								<template slot-scope="scope">
									<el-input-number v-model="scope.row.topLevel" controls-position="right" @change="handleChange" :min="1" :max="99" size="mini"></el-input-number>
								</template>
							</el-table-column>
							...
							<el-table-column label="操作" prop="id" width="160" align="center">
								<template slot-scope="scope">
									<cms-button v-show="scope.row.hasUpdateRight" type="edit"
										@click.native="routerLink('/content/edit','update',scope.row.id)"
										v-perms="'/content/update'"></cms-button>
									<cms-button v-show="scope.row.hasDeleteRight" type="delete"
										@click.native="deleteBatch($api.contentDelete,scope.row.id)"
										v-perms="'/content/delete'"></cms-button>
									<el-dropdown size="small" placement="bottom-end" class="ml-10">
										<el-button type="warning" icon="el-icon-more" title="更多操作" size="mini" circle>
										</el-button>
										<el-dropdown-menu slot="dropdown">
											<el-dropdown-item>
												<el-button size="mini" :disabled="disabled"
													@click="batch($api.contentStatic)" type="text"
													v-perms="'/content/contentStatic'">生成静态页</el-button>
											</el-dropdown-item>
											<el-dropdown-item divided>
												<el-button size="mini" :disabled="disabled" @click="siteVisble=true"
													type="text" v-perms="'/content/contentPush'">推送</el-button>
											</el-dropdown-item>
										</el-dropdown-menu>
									</el-dropdown>
								</template>
