				...
				<el-form ref="form" :model="info" :rules="rules" class="cms-form" label-width="162px"
					...
					<el-dialog class="dialog" title="选择副栏目" :visible.sync="channelVisble" width="30%">
						<div class="tree-layout">
							<div class="tree">
								<div class="tree">
									<el-tree :data="treeData" ref="channelTree" :props="props"
										@check-change="checkChange" :default-expanded-keys="['']" show-checkbox
										:indent="16" node-key="id">
									</el-tree>
								</div>
							</div>
						</div>
					...
					<template v-for="(item,index) in itemList">
						<el-form-item :key="index" v-if="item.field=='title'" class="flex-100" :label="item.label"
							prop="title">
					...
					<!-- 系统字段 -->
					<template v-for="(item,index) in itemList">
						<el-form-item :key="index" :label="item.label" :class="[item.single?'flex-100':'flex-50']"
                ...
                &&item.field!='origin'"></el-input>
                 ...
                 "></el-input>
							...
							<div v-if="item.field=='topLevel'">
								...
								<div class="content-inner-box">
									...
									<div style="margin-top:15px">
										...
										<el-date-picker class="w120" type="datetime" value-format="yyyy-MM-dd HH:mm:ss"
											v-model="info['sortDate']" style="width:195px">
										</el-date-picker>
									</div>
								</div>
							</div>
								...
								<div>
									...
									<div class=" mt-10" v-if="info['rewardPattern']==true&&info['charge']==2">
										...
										<el-input type="number" class="w50" v-for="item in chargeMap" :key="item.key"
											v-model="item.value" style="margin-right:5px;"></el-input>
									</div>
								</div>
							</div>
					...
					<div class="form-footer">
						<el-button v-if="this.id==0" type="warning" @click="add(true)" v-perms="'/content/save'">提交并继续添加
						</el-button>
						<el-button v-if="this.id==0" type="primary" @click="add(false)" v-perms="'/content/save'">提交
						</el-button>
						<el-button type="primary" @click="add(true,true)" v-perms="'/content/save'">预览</el-button>
			...
			getAllList() {
				...
				this.$axios
					.all([
						...
						this.$axios.post(api.configContentChargeGet)
					])
					.then(
						this.$axios.spread((channels, topicList, type, groups, tplAll, charge) => {
